...
namespace IronRuby.Aot.Compiler {
    ...
    internal partial class StackSpiller {
        ...
        internal static LambdaExpression AnalyzeLambda(LambdaExpression lambda) {
            return new StackSpiller(Stack.Empty).Rewrite(lambda);
        }
        ...
        private Result RewriteDynamicExpression(Expression expr, Stack stack) {
            ...
            return cr.Finish(cr.Rewrite ? node.Update(cr[0, -1]) : expr);
        }
        ...
        private Result RewriteLogicalBinaryExpression(Expression expr, Stack stack) {
            ...
            if (action != RewriteAction.None) {
                ...
                expr = Expression.MakeBinary(node.NodeType, left.Node, right.Node, node.IsLiftedToNull, node.Method, (LambdaExpression)conversion.Node);
            }
        ...
        private Result RewriteBinaryExpression(Expression expr, Stack stack) {
            ...
            return cr.Finish(cr.Rewrite ?
                                    Expression.MakeBinary(node.NodeType, cr[0], cr[1], node.IsLiftedToNull, node.Method, (LambdaExpression)cr[2]) :
                                    expr);
        }
        ...
        private Result RewriteMemberExpression(Expression expr, Stack stack) {
            ...
            if (expression.Action != RewriteAction.None) {
                ...
                expr = Expression.MakeMemberAccess(expression.Node, node.Member);
            }
        ...
        private Result RewriteInvocationExpression(Expression expr, Stack stack) {
            ...
            return cr.Finish(cr.Rewrite ? Expression.Invoke(cr[0], cr[1, -1]) : expr);
        }
        ...
        private Result RewriteNewExpression(Expression expr, Stack stack) {
            ...
            return cr.Finish(cr.Rewrite ? (node.Members == null ? Expression.New(node.Constructor, cr[0, -1]) : Expression.New(node.Constructor, cr[0, -1], node.Members)) : expr);
        }
        ...
        private Result RewriteUnaryExpression(Expression expr, Stack stack) {
            ...
            if (expression.Action != RewriteAction.None) {
                expr = Expression.MakeUnary(node.NodeType, expression.Node, node.Type, node.Method);
            }
            return new Result(expression.Action, expr);
        }
        ...
        private Result RewriteBlockExpression(Expression expr, Stack stack) {
            ...
            if (action != RewriteAction.None) {
                ...
                expr = Expression.Block(node.Type, node.Variables, clone);
            }
            return new Result(action, expr);
        }
        ...
        private Result RewriteLoopExpression(Expression expr, Stack stack) {
            ...
            if (action != RewriteAction.None) {
                expr = Expression.Loop(body.Node, node.BreakLabel, node.ContinueLabel);
            }
            return new Result(action, expr);
        }
        ...
        private Result RewriteSwitchExpression(Expression expr, Stack stack) {
            ...
            for (int i = 0; i < cases.Count; i++) {
                ...
                if (body.Action != RewriteAction.None || cloneTests != null) {
                    ...
                    @case = Expression.SwitchCase(body.Node, testValues);
            ...
            if (action != RewriteAction.None) {
                ...
                expr = Expression.Switch(node.Type, switchValue.Node, defaultBody.Node, node.Comparison, cases);
            }
        ...
        private Result RewriteTryExpression(Expression expr, Stack stack) {
            ...
            if (action != RewriteAction.None) {
                ...
                expr = Expression.MakeTry(node.Type, body.Node, @finally.Node, fault.Node, handlers);
            }
            return new Result(action, expr);
        }
