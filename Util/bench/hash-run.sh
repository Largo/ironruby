#!/bin/bash
# Times IronRuby (./ir.sh) against CRuby (`ruby`) on Util/bench/hash-bench.rb and prints ratios.
#
...
#
#   Util/bench/hash-run.sh                          # every benchmark, best of 3
#   Util/bench/hash-run.sh 5 out.txt insert_sym     # just one, best of 5, tee'd to out.txt
#   IR=/path/to/ir.sh Util/bench/hash-run.sh        # measure a different build
#   ./ir.sh Util/bench/hash-bench.rb --list         # the benchmark names
#
# A benchmark takes an optional scale factor as its second argument
# (`ruby Util/bench/hash-bench.rb insert_sym 10`), which multiplies the iteration counts.
...
shift 2 2>/dev/null
BENCHES="$*"
[ -z "$BENCHES" ] && BENCHES=$("$RUBY" "$HERE/hash-bench.rb" --list)
TIMEOUT="${TIMEOUT:-600}"
...
for bn in $BENCHES; do
  c=$(best "$RUBY" "$HERE/hash-bench.rb" "$bn")
  r=$(best "$IR" "$HERE/hash-bench.rb" "$bn")
