...
module Psych
    ...
    class Object
        def self.tag_subclasses?; false; end
        def to_yaml( opts = {} )
            Psych::quick_emit( self, opts ) do |out|
            ...
            if Array === val
                val.each do |v|
                    if Hash === v
                        ...
                        raise Psych::Error, "Invalid !omap entry: " + val.inspect
                    end
                end
            else
                raise Psych::Error, "Invalid !omap: " + val.inspect
            end
            self
        end
        def self.[]( *vals )
            ...
            if Array === val
                val.each do |v|
                    if Hash === v
                        ...
                        raise Psych::Error, "Invalid !pairs entry: " + val.inspect
                    end
                end
            else
                raise Psych::Error, "Invalid !pairs: " + val.inspect
            end
            self
        end
        def self.[]( *vals )
