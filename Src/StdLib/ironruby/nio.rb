...
module NIO
  ...
  def self.pure?(env = ENV)
    ...
    # (IronRuby: its native half is C#, and runs on Windows too.)
    if (Gem.win_platform? && !defined?(JRUBY_VERSION) && RUBY_ENGINE != "ironruby")
      return true
    end
...
if NIO.pure?
  ...
  if defined?(JRUBY_VERSION)
    ...
    # IronRuby: nio4r_ext is Src/Libraries/Nio4r, poll(2) / Socket.Select rather than libev.
    NIO::ENGINE = "dotnet"
  else
    NIO::ENGINE = "libev"
  end
