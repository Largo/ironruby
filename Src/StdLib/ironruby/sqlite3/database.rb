...
#
# Vendored from the sqlite3 gem 2.9.6 (lib/sqlite3/database.rb), whose Ruby half sits
...
module SQLite3
  ...
  class Database
    ...
    def initialize file, options = {}, zvfs = nil
      ...
      # IronRuby: sqlite3_open16 is not bound (SQLitePCL.raw does not expose it, and
      # .NET has no UTF-16 filesystem API on Unix anyway).  A UTF-16 filename is
      # transcoded and opened through sqlite3_open_v2, which is the same file.
      if file.encoding == ::Encoding::UTF_16LE || file.encoding == ::Encoding::UTF_16BE || options[:utf16]
        open_v2 file.encode("utf-8"), Constants::Open::READWRITE | Constants::Open::CREATE, zvfs
      else
    ...
    def encoding
      # IronRuby: the gem's C extension answers sqlite3_db_encoding(); the same
      # answer comes out of the pragma, which is one query on the first call.
    ...
    def authorizer(&block)
      # IronRuby: sqlite3_set_authorizer is not bound; see the "Not implemented"
      # section at the bottom of this file.
      raise NotImplementedError, "SQLite3::Database#authorizer is not implemented on IronRuby"
    end
    ...
    # IronRuby: the gem's C extension defines define_aggregator2(klass, name);
    # here the primitive underneath is #define_aggregate_function, which keeps the
    # dispatch to the handler object in Ruby.  +klass+ answers .arity and .new, and
    # an instance answers #step(*args) and #finalize, as the three callers above
    # (#create_aggregate, #create_aggregate_handler, #define_aggregator) arrange.
    def define_aggregator2(klass, name) # :nodoc:
      define_aggregate_function(name, klass.arity,
        proc { klass.new },
        proc { |instance, *args| instance.step(*args) },
        proc { |instance| instance.finalize })
      self
    end

    # --- IronRuby: not implemented -----------------------------------------
    #
    # These are the parts of the gem's C extension that SQLitePCL.raw - the
    # binding underneath - does not expose.  They raise rather than quietly do
    # nothing, so that a program relying on them says so.
    #
    # #busy_handler is implemented, but differently: see the comment on
    # Statement#step in Src/Libraries/Sqlite3/Sqlite3.cs.
    %i[authorizer= trace statement_timeout=].each do |name|
      define_method(name) do |*_args, &_block|
        raise NotImplementedError, "SQLite3::Database##{name} is not implemented on IronRuby"
      end
    end
