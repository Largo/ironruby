...
module URI
  class RFC3986_Parser # :nodoc:
    ...
    # IronRuby: .NET regexes have no \g<name> subexpression calls, so the
    # upstream HOST and RFC3986_URI/relative_ref patterns are written out with
    # every \g replaced by a copy of the group it calls. None of the calls is
    # recursive, so the patterns match exactly the same strings.
    DEC_OCTET = %r[(?:[1-9]\d|1\d{2}|2[0-4]\d|25[0-5]|\d)].source
    IPV4ADDRESS = %r[#{DEC_OCTET}\.#{DEC_OCTET}\.#{DEC_OCTET}\.#{DEC_OCTET}].source
    LS32 = %r[(?:\h{1,4}:\h{1,4}|#{IPV4ADDRESS})].source
    HOST = %r[
      (?<IP-literal>\[(?:
          (?<IPv6address>
            (?:\h{1,4}:){6}
          ...
          | ::(?:\h{1,4}:){5}#{LS32}
          | \h{1,4}?::(?:\h{1,4}:){4}#{LS32}
          | (?:(?:\h{1,4}:)?\h{1,4})?::(?:\h{1,4}:){3}#{LS32}
          | (?:(?:\h{1,4}:){,2}\h{1,4})?::(?:\h{1,4}:){2}#{LS32}
          | (?:(?:\h{1,4}:){,3}\h{1,4})?::\h{1,4}:#{LS32}
          | (?:(?:\h{1,4}:){,4}\h{1,4})?::#{LS32}
          | (?:(?:\h{1,4}:){,5}\h{1,4})?::\h{1,4}
          | (?:(?:\h{1,4}:){,6}\h{1,4})?::
          )
